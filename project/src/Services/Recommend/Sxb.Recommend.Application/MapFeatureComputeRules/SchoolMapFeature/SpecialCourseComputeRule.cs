...
{
    public class SpecialCourseComputeRule : ISchoolComputeRule
    {
        public async Task<double> Compute(School schoolP, School schoolS, MapFeature schoolMapFeature)
        {

            if (schoolP.SpecialCourse.Any(p => schoolS.SpecialCourse.Any(s => s.Equals(p, StringComparison.OrdinalIgnoreCase))))
            {
